...
def test_notebook_replacement_purges_execution_and_attempt_lookups():
    class ReplacementKernel(FakeKernel):
        max_output_items = 1000
        max_output_bytes = 5 * 1024 * 1024
    ...
    snapshot = documents.import_notebook(notebook("x = 1"))
    service = KernelExecutionService(
        documents=documents, kernel=ReplacementKernel(),
    )
    service._kernel_notebook_session_id = snapshot.session_id
    operation = wait_terminal(service, service.start_cell(
        cell_id="cell-0", session_id=snapshot.session_id,
    ...
    replacement = documents.import_notebook(
        notebook("y = 2"), expected_session_id=current.session_id,
        expected_revision=current.revision,
    )
