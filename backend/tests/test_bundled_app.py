...
def test_defaults_to_the_configured_agent_adapters(dist, monkeypatch):
    ...
    monkeypatch.setenv("NOTEBOOK_DEFAULT_AGENT", "claude")
    app = create_bundled_app(dist_dir=dist)
    service = app.state.api.state.agent_turn_service
    assert isinstance(service.adapter, ClaudeAgentAdapter)
    # The picker offers the same set here as in a developer run, rather than
    # collapsing to whichever agent happens to be the default.
