...
// accepted in §H, and `.outline-state` is already absent from the stylesheet).
// `block.state` still arrives from the backend and still classes the row, so
// bringing a marker back is a render change and not a data one.
//
// Worth knowing before that happens: the segmenter no longer draws a boundary
// where the run-state changes, so an unrun tail can sit inside a block that
// reports "ok". See test_simulation_sweep_function_layer.
