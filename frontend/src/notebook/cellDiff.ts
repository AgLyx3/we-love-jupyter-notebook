...
export type CellDiffRanges = { added: number[]; removed: { line: number; text: string }[] };
export function cellDiffRanges(before: string, after: string): CellDiffRanges {
  if (before === after) return { added: [], removed: [] };
  const added: number[] = [];
  const removed: { line: number; text: string }[] = [];
  let line = 0;
  for (const entry of lineDiff(before, after)) {
    if (entry.kind === "same") line += 1;
    else if (entry.kind === "added") { added.push(line); line += 1; }
    else removed.push({ line, text: entry.text });
  }
