...
export default function NotebookView({ notebook, scope, turn, tuningRecord = null, trusted = false, kernel, disabled, sourceActionsDisabled, autoSave, focusRequest, outlinedCellIds, tuningControls, tunableCellIds, onDirtyChange, onSave, onRun, onScope, onScopeMany, onRevert, onKeepCell, onKeepOperation, onUndoOperation, onKeepTuned, onUndoTuned, onAddSelectionToChat, onInlineEdit, onAddErrorToChat }: {
  ...
  return <main ref={surfaceRef} className="notebook-surface" aria-label="Notebook cells" onPointerDown={startMarquee} onKeyDown={(event) => {
    ...
    {menu && <div className="context-menu-backdrop" onClick={() => setMenu(null)} onContextMenu={(event) => { event.preventDefault(); setMenu(null); }}>
      <div className="cell-context-menu" style={{ left: menu.x, top: menu.y }} role="menu" aria-label="Cell scope actions" onClick={(event) => event.stopPropagation()}>
        ...
        <button role="menuitem" disabled={scopeDisabled} onClick={() => applyScope(false)}>Add {selected.size} as focus</button>
