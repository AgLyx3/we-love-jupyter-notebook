  ...
  return <div className="dialog-backdrop" onClick={onClose}>
    <section className="file-picker" role="dialog" aria-modal="true" aria-label={mode === "save" ? "Save notebook as" : "Open a notebook or folder"} onClick={(event) => event.stopPropagation()}>
      <header className="file-picker-head">
        <button disabled={!listing?.parent} title="Parent folder" aria-label="Parent folder" onClick={() => navigate(listing?.parent ?? undefined)}><Icon name="arrow_upward" /></button>
        <span className="file-picker-path" title={listing?.path}>{listing?.path ?? "…"}</span>
        <button title="Close" aria-label="Close file picker" onClick={onClose}><Icon name="close" /></button>
      </header>
      ...
      <ul className="file-picker-list" aria-label="Folder contents">
        ...
        {!loading && listing?.entries.map((entry) => <li key={entry.path}>{entry.kind === "directory"
          ? <button className="file-picker-row" onClick={() => navigate(entry.path)}><Icon name="folder_open" /> {entry.name}</button>
          : entry.kind === "notebook"
          ? <button className="file-picker-row notebook" onClick={() => mode === "save" ? setName(entry.name) : onOpenNotebook?.(entry.path, root ?? listing.path)}><Icon name="description" /> {entry.name}</button>
          : <span className="file-picker-row other" aria-disabled="true" title={`${entry.name} — not a notebook; shown for reference, can't be opened here`}><Icon name="description" /> {entry.name}</span>}</li>)}
      </ul>
      {mode === "save"
