...
### 10.3 Revisions after the second review pass

Five findings, each reproduced as a failing test before it was fixed. Four were
the feed stating something the document contradicted; the fifth was a bound that
bounded nothing.

| # | Decision | Alternatives considered | Why |
|---|---|---|---|
| D31 | A whole-turn undo overrides per-operation state: every entry reports `UNDONE`, and the cell is not called stale | Read the ledger as before; leave `undone_at` consulted only in the no-coverage branch | `undo()` restores the pre-turn checkpoint, so nothing the turn wrote survives — but it settles only the operations still `pending`, so a hunk the user had accepted keeps `accepted` and rendered `KEPT`, "the result is in notebook.ipynb", for content the undo had just removed. That is the direction D24 already named as the dangerous one. Staleness moved with it: composing the ledger against the restored document made every such cell report stale the instant it was undone, with nothing having touched it |
| D32 | Staleness is derived over `operations` ∪ `changes`, and a change with no ledger coverage expects `next_source` | Iterate `operations` only (as before); emit no entry for uncovered cells | The feed emits an entry for every cell in `changes`, but staleness iterated the ledger, so a retyped cell in a Trusted turn — a change with no operation, by design — could never be flagged however far the document moved on. `compose` must not be asked for these: with no operations it rebuilds `previous_source`, which would call every such cell stale the moment the turn landed |
| D33 | `delete`, `move` and `retype` get memory entries from `structural_ops`, stated rather than diffed | Leave them out; carry the deleted cell's source so the feed can show it | A deleted cell has no "after" so it is absent from `changes`, and delete is not a ledger kind so it is absent from `operations`. A delete-only Trusted turn therefore hit the empty-operations branch and rendered "It made no changes" — a false statement about the most destructive edit available, and the same failure D26 fixed for adds. Content is not carried: the turn does not retain the frozen sources past the apply, and copying them on would grow the retained history without `_history_size` seeing it. A retype whose source also changed is left to its `changes` entry rather than reported twice |
| D34 | Operations the history-budget evictor settles are recorded on the turn and report `APPLIED`, not `KEPT` | Leave them as `accepted`; give them a status of their own | `_force_settle_locked` marks unreviewed operations `accepted` so they stop advertising review controls that are gone. By state alone that is indistinguishable from a person approving them, so the feed reported `KEPT` and invented the consent D21 exists to refuse to invent. `APPLIED but not yet reviewed` is exactly true of them: in the notebook, seen by nobody |
| D35 | `MEMORY_BUDGET_BYTES` bounds the whole render; an oversized newest turn is truncated, not dropped | Leave the newest block exempt; shrink `MEMORY_DIFF_LINES`; cap each line's length | The newest block was admitted at any size, deliberately — a feed that drops the turn that just happened is worse than one that abbreviates it — but nothing then bounded it. One turn carries an entry per edited cell, each with up to `MEMORY_DIFF_LINES` rows whose line length has no bound: twenty undone cells of long lines render half a megabyte against a 16 KiB budget, straight into the next turn's `INSTRUCTIONS.md`. Truncating keeps the newest turn present and the bound absolute. The wrapper lines are counted too, so the budget holds in every case rather than most |
